...
class FirebaseService {
  ...
  static Stream<BusinessPlanSubscription?> businessPlanStreamForUser(String ownerId) {
    try {
      return _firestore
        ...
        final rawData = doc.data() as Map<String, dynamic>;
        final data = _convertFirestoreData(rawData);
        return BusinessPlanSubscription.fromJson(doc.id, data);
      });
    } catch (e) {
      debugPrint('❌ Erro ao criar stream de plano de negócio: $e');
      return const Stream.empty();
    }
  }

  static Stream<List<BusinessPlanSubscription>> businessPlansStreamForOwner(
      String ownerId) {
    try {
      return _firestore
          .collection('businessPlans')
      ...
      debugPrint('❌ Erro ao criar stream de planos de negócio: $e');
      return const Stream.empty();
    }
  }

  static Future<BusinessPlanSubscription?> getCurrentBusinessPlanForUser(String ownerId) async {
    try {
      final querySnapshot = await _firestore
          .collection('businessPlans')
          .where('ownerId', isEqualTo: ownerId)
      ...
      debugPrint('❌ Erro ao buscar plano de negócio do usuário: $e');
      return null;
    }
  }

  static Future<BusinessPlanSubscription?> getCurrentBusinessPlanForEstablishment({
    required String ownerId,
    required String establishmentId,
  }) async {
    try {
      ...
      if (querySnapshot.docs.isEmpty) {
        // Fallback: pode ser um plano mais antigo sem establishmentId vinculado.
        return await getCurrentBusinessPlanForUser(ownerId);
      }
      ...
      debugPrint('❌ Erro ao buscar plano de negócio do estabelecimento: $e');
      try {
        // Fallback adicional em caso de erro de índice ou outro problema na query específica
        return await getCurrentBusinessPlanForUser(ownerId);
      } catch (e2) {
        debugPrint('❌ Erro ao buscar plano de negócio do usuário como fallback: $e2');
        return null;
      }
    }
  }

  static Future<BusinessPlanSubscription?> createBusinessPlanRequest({
    required String ownerId,
    required String establishmentId,
    ...
    try {
      final docRef = await _firestore.collection('businessPlans').add({
        'ownerId': ownerId,
        'establishmentId': establishmentId,
        'planType': planType.toString().split('.').last,
        'status': BusinessPlanStatus.pendingApproval.toString().split('.').last,
      ...
      final rawData = snapshot.data() as Map<String, dynamic>;
      final data = _convertFirestoreData(rawData);
      return BusinessPlanSubscription.fromJson(docRef.id, data);
    } catch (e) {
      debugPrint('❌ Erro ao criar solicitação de plano de negócio: $e');
      return null;
    }
  }

  static Future<void> applyBusinessPlanToEstablishments(
    String ownerId,
    BusinessPlanSubscription? plan,
  ) async {
    try {
      final querySnapshot = await _firestore
          .collection('establishments')
      ...
      final bool shouldBoost = isActivePlan && plan!.planType != BusinessPlanType.basic;

      DateTime? boostExpiresAt;
      if (shouldBoost) {
        boostExpiresAt = plan.validUntil;
      }
      ...
      for (final doc in querySnapshot.docs) {
        ...
        if (!shouldBoost) {
          ...
          updates['boostExpiresAt'] = boostExpiresAt?.toIso8601String();
        }
      ...
      debugPrint('✅ Plano business aplicado para estabelecimentos de $ownerId (boost=${shouldBoost ? 'on' : 'off'})');
    } catch (e) {
      debugPrint('❌ Erro ao aplicar plano business nos estabelecimentos: $e');
    }
  }
